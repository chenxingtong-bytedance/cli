...
func TestChatMessagesListCitationsIncludesTextThreadReplies(t *testing.T) {
	...
	if got[0].ResourceID != "om_1" {
		t.Errorf("first resource_id = %q, want parent message", got[0].ResourceID)
	}
	if got[1].ResourceID != "om_reply_1" || got[1].Title != "first reply" {
	...
	if got[2].ResourceID != "om_reply_2" || got[2].Title != "second reply" {
